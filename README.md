# 2022-FIFA-World-Cup-Analysis
Updating and analyzing data from 2022 FIFA World Cup.

## 📝 Short Description
This project focuses on exploratory data analysis of the FIFA World Cup 2022 matches using SQL.

The goal of the analysis was to explore match outcomes, goals scored, and host's performance during World Cup. The analysis provides answers to several analytical questions related to:
- Performance of host countries historically
- Poland's performance in the tournament
- Average goals scored in World Cups and goal records
- Overall match statistics and historical comparisons

The project uses SQL queries to update missing data, correct inaccurate values and perform data aggregation, as well as analysis. Results of the queries are interpreted in the comments.

## 🛠️ Tools used
- SQL (MySQL) - queries
- DBeaver - data import

## 💾 Data Source
- Source: FIFA
- File format: CSV
- Data structure: Two tables
- Number of records: 86

## ❔ Key Questions
- How have World Cup host countries performed historically? Did Qatar follow a similar path?
- Where did Poland finish the 2022 World Cup?
- What is the average number of goals scored in a World Cup? Was the number of goals in the Qatar World Cup higher or lower than the average?
- Historically, which World Cup had the highest number of goals?

## 📊 Results/ Insights
**1. Host Results**
 
![Host Results](https://github.com/radektadla/2022-FIFA-World-Cup-Analysis/blob/main/Results%20Preview/Host%20Results.png)

Short summary: Historically, host nations finished in the top four in **59% of World Cups (13 out of 22)**.
However, **Qatar finished outside the top four**, performing worse than most host countries.

**2. Poland Performance**

![Poland Performance](https://github.com/radektadla/2022-FIFA-World-Cup-Analysis/blob/main/Results%20Preview/Poland%20Performance.png)

Short summary: Poland reached the **Round of 16**, where they lost **1–3 to France**.

**3. Goals Analysis**

![Goals Analysis](https://github.com/radektadla/2022-FIFA-World-Cup-Analysis/blob/main/Results%20Preview/Goal%20Analysis.png)

Short summary: The average number of goals scored in World Cups is **123.64**.
The **Qatar 2022 World Cup recorded 172 goals**, the highest number in Wold Cup history.

---------

All code, distinct queries, as well as more detailed answers and in-depth analysis can be found in the SQL scripts included in this repository.

Thank you for reading and checking out my project!
